...
public class CompanyServiceImpl implements CompanyService {
	...
	@Override
	public CompanyServiceModel editCompanyInfo(CompanyServiceModel companyServiceModel) {

		Company company = this.companyRepository.findById(companyServiceModel.getId()).orElseThrow(() -> new NoSuchElementException(Constants.COMPANY_ERROR_MESSAGE));

		companyServiceModel.setStatus(company.getStatus().toString());

		company = this.companyRepository.saveAndFlush(this.modelMapper.map(companyServiceModel, Company.class));

		CompanyServiceModel editedcompanyServiceModel = this.modelMapper.map(company, CompanyServiceModel.class);

		return editedcompanyServiceModel;
	}
	...
	public CompanyServiceModel findCompanyByID(String id) {
		...
		Company company = this.companyRepository.findById(id).orElseThrow(() -> new NoSuchElementException(Constants.COMPANY_ERROR_MESSAGE));
